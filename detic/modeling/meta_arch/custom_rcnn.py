...
class CustomRCNN(GeneralizedRCNN):
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if self.with_image_labels:
            if ann_type in ['box', 'prop', 'proptag']:
                ...
                losses.update({k: v * 0 if k != 'loss_centernet_loc' or v.item() == 0
                else v for k, v in proposal_losses.items()})
        else:
            losses.update(proposal_losses)
    ...
    def _sample_cls_inds(self, gt_instances, ann_type='box'):
        if ann_type == 'box':
            ...
            if gt_classes.max() > C and C == 233:
                freq_weight = load_class_freq(
                'datasets/360/11_pl_final_cat_info.json',
                0.5)
                C = 466
        else:
            gt_classes = torch.cat(
