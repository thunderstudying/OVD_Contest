...
from mmdet.models.losses.iou_loss import CIoULoss, EIoULoss, SIoULoss
...
class DeticFastRCNNOutputLayers(FastRCNNOutputLayers):
    ...
    def box_reg_loss(
        ...
        if self.box_reg_loss_type == "smooth_l1":
            ...
            ciou = CIoULoss(eps=1e-7, reduction='sum')
            loss_box_reg = ciou(fg_pred_boxes, gt_boxes[fg_inds])
        elif self.box_reg_loss_type == "eiou":
            fg_pred_boxes = self.box2box_transform.apply_deltas(
                fg_pred_deltas, proposal_boxes[fg_inds]
            )
            eiou = EIoULoss(eps=1e-7, reduction='sum')
            loss_box_reg = eiou(fg_pred_boxes, gt_boxes[fg_inds])
        elif self.box_reg_loss_type == "siou":
            fg_pred_boxes = self.box2box_transform.apply_deltas(
                fg_pred_deltas, proposal_boxes[fg_inds]
            )
            siou = SIoULoss(eps=1e-7, reduction='sum')
            loss_box_reg = siou(fg_pred_boxes, gt_boxes[fg_inds])
        else:
