...
from detic.modeling.utils import reset_cls_test, ModelEMA
...
def do_train(cfg, model, resume=False):
    ...
    if cfg.MODEL_EMA:
        ema = ModelEMA(model) if comm.is_main_process() else None
    ...
    with EventStorage(start_iter) as storage:
        ...
        for data, iteration in zip(data_loader, range(start_iter, max_iter)):
            ...
            if cfg.MODEL_EMA:
                additional = {'ema': deepcopy(ema.ema.state_dict())}
                periodic_checkpointer.step(iteration, **additional)
